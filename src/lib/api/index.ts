import { z, ZodTypeAny, ZodObject, ZodArray, ZodOptional, ZodNullable, ZodRecord, ZodUnion, ZodLazy, ZodEnum, ZodNumber, ZodBoolean } from "zod"
...
import { ChainQueryParamsSchema, ChainResponseSchema } from "@/src/lib/schemas/chain"
import {
...
const getUnionType = (schema: z.ZodUnion<any>): string => {
	const types = schema._def.options.map((opt: ZodTypeAny) => getZodTypeName(opt))
...
const ENDPOINT_DEFINITIONS: EndpointDefinition[] = [
	{
		...
		path: "/{address}",
		summary: "Get address",
		description: "Returns all address level metadata for the provided EVM address including signal-level graph flags.",
		tag: "address",
	...
	},
	{
		...
		description: "Returns all indexed fungible, non-fungible, and non-tokenized positions for the provided address.",
		tag: "address",
		pathParams: AddressParamsSchema,
	...
	},
	{
		...
		path: "/{address}",
		summary: "Get context",
		description: "Returns all the options needed for each action input to build a valid transaction such as tokens, pools, and chains.",
		tag: "transaction",
		pathParams: AddressParamsSchema,
	...
	},
	{
		...
		description: "Returns all transactions associated with the EVM address provided, after considering all applied filters.",
		tag: "transaction",
		pathParams: AddressParamsSchema,
	...
	},
	{
		...
		description: "Constructs the transaction calldata and returns the transaction details ready for signing and submission.",
		tag: "transaction",
		pathParams: AddressParamsSchema,
	...
	},
	{
		...
		path: "/chain",
		summary: "Get chain(s)",
		description: "Returns all data for a specific blockchain network including its configuration and metadata.",
		tag: "chain",
...
const buildEndpoint = (def: EndpointDefinition): ApiEndpoint => {
	...
	if (def.pathParams) {
		params.push(...flattenParamsFromSchema(def.pathParams, "path", "", ADDRESS_EXAMPLE))
	}
...
export const API_ENDPOINTS: ApiEndpoint[] = ENDPOINT_DEFINITIONS.map(buildEndpoint)
