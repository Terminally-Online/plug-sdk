...
export const ContextStepSchema = z.object({
	...
	// When this action's definition last looked different, measured by the API
	// rather than declared: it digests what it serves and remembers when each
	// digest first appeared. Optional because a definition the API has not
	// recorded yet carries no date, and no date is the honest answer where there
	// is nothing to say.
...
export const ContextFilterSchema = z.object({
	...
	protocol: z
		.string()
		.optional()
		.describe("Filter results to specific protocols from the listed of supported options."),
...
export type ContextActionAttributes = z.infer<typeof ContextStepAttributesSchema>
export type ContextActionOutputInfo = z.infer<typeof ContextStepOutputInfoSchema>
...
export type ContextQueryParams = AddressParams & z.infer<typeof ContextQueryParamsSchema>
